...
### 10. 自適應流程錯誤降級或過度 fan-out

專案小、檔案少或行數低不代表風險低。若 assessment 遺漏 hard escalation、
把缺失證據當作 `COMPACT`、以 model 名稱授權、在 ownership 重疊時增加 lane，
或讓 implementation owner 控制 helper，就會用「效率」繞過必要的安全與責任
邊界。反向地，低風險單一變更若無理由套用完整 high-assurance ceremony，則是
可維護性與成本缺陷。

**TDD 必要案例**：表格化測試每一 assessment factor 與 hard escalation；至少
包含小型 privileged-XSS／付款／Secret 工單強制 `HIGH_ASSURANCE`、缺失證據
不得 `COMPACT`、一般單一可逆改動使用一位 implementer、重疊檔案拒絕多 lane、
互斥 ticket 才允許平行、不可用 model tier fail closed、helper 預設為零且只能由
reviewer 建立為 read-only/no-code。逐一反向移除 escalation、default-one、
disjoint-ownership 與 reviewer-only helper gate 時，其對應測試必須轉紅。

**CR 必要動作**：

1. 依實際 diff 與 effect boundary 重算 profile，逐一記錄 assessment factor 與 hard escalation。
2. 核對 implementer/model/helper 數量是否為最小安全集合；任何多 lane 必須逐一對到互斥 ownership、獨立 AC 與整合順序。
