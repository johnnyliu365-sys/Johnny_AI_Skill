...
| Ticket 05B4 registration composition parent | CONVERGENCE_DECOMPOSED / CHILD_05B4B1_REVISION_02_CORRECTION_PENDING / CHILD_05B4B2_DEPENDENCY_WAIT | CR-148 is closed; CR-149 requires one bounded B1 validation correction before B2. |
...
| Ticket 05B4B1 pure registration reducer | IN_PROGRESS / REVISION_02_CHANGES_REQUESTED / CORRECTION_PENDING | CR-148 closed; missing `status` on constructed-invalid pending values must return finite `INVALID_STATE` instead of raising. |
| Ticket 05B4B2 transaction coordinator | PLANNED / DEPENDENCY_WAIT / REFREEZE_REQUIRED | Sole future owner of attempt/phase/generation/one-shot lease and effect replay exclusion; refreeze after B1 revision 02 integration. |
