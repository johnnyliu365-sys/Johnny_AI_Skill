...
- `RESEARCH_HELPER` is reviewer-owned, read-only and no-code as defined by `delivery-profile.md`.
  An ordinary research helper is not admitted for `COMPACT`; `STANDARD` may use one optional
  ordinary read-only helper. A reviewer-owned `AdversarialReviewPlan` may consume exactly one
  helper at `COMPACT`/`STANDARD` when optional, and must consume one at `HIGH_ASSURANCE` or before
  a proposed release/deployment; it returns finite evidence only and cannot approve, integrate,
  dispatch, modify code or perform an external effect.
