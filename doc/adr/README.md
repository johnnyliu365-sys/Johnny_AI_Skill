...
| [ADR-20260827-022-lock-bound-telemetry-storage-admission.md](ADR-20260827-022-lock-bound-telemetry-storage-admission.md) | `ACCEPTED` | 將 Context Governor 的 durable storage 收斂為 exact-identity lock、持鎖重驗與無副作用 contention。 |
...
| [ADR-20260828-031-procedural-managed-artifact-behavior.md](ADR-20260828-031-procedural-managed-artifact-behavior.md) | `PROPOSED / OWNER_DECISION_REQUIRED` | 將文件與直接父索引收斂為單一受管操作，以 host behavior adapter 建立回饋迴圈，並保留 repository gate 為最終 authority。 |
