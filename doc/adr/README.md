# Architecture Decision Records

僅在出現重大且難以回復的決策時，才在此建立 ADR。

| ADR | 狀態 | 用途 |
| --- | --- | --- |
| [ADR-20260804-001-private-router-saas.md](ADR-20260804-001-private-router-saas.md) | `SUPERSEDED` for future direction | 保留歷史 private Router SaaS POC 的決策與邊界。 |
| [ADR-20260805-002-autonomous-collaboration-control-plane.md](ADR-20260805-002-autonomous-collaboration-control-plane.md) | `PROPOSED — input to Grill` | 定義非商用多 AI 協作／稽核控制平面的高階架構。 |
