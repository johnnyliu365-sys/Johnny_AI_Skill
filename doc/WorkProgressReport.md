...
## PRG-20260811-141 — Ticket 06A implementation return
...
| First red | Before either implementation module existed, the four P1–P4 test methods each failed with `ModuleNotFoundError` for the required contracts/probe modules. A later P4 profile-directory conflict first raised `FileExistsError`; the final implementation now performs owned-root teardown and its regression is green. |
| P1 / P3 evidence | Frozen strict reviewer and implementation profiles render only `name`, `description`, `developer_instructions`, and finite `agents.enabled` policy; reviewer is `true`, implementation is `false`. Config text alone returns `INSTALL_BLOCKED`; the suite covers both direct and indirect reviewer/implementation surfaces, reversed policies, forged bindings and malformed readback. |
| P2 actual capability | Exact WindowsApps `codex.exe` discovery was explicit with no wildcard fallback. One bounded disposable `--version` invocation through integrated 05S1/05S2 returned finite `ACCESS_DENIED`; no model, network, live user Codex home or target repository was touched. Because 05S2 intentionally preserves no child stdout and no deterministic host capability readback exists, the actual result is `INSTALL_BLOCKED / ROLE_ISOLATION_UNPROVEN / OUTPUT_UNAVAILABLE`, not `SUPPORTED`. |
| P4 / residue evidence | Every normal and failed probe uses a leased disposable `CODEX_HOME`; final actual readback reported `REMOVED`, root absent and both profile files absent. Tests preserve a same-name foreign sentinel plus byte and porcelain snapshots of temporary existing/empty representative repositories. Cleanup-failure and profile-write-failure paths fail closed. |
| Verification | Focused `unittest` 11/11; full discovery 206/206; strict `mypy --strict --explicit-package-bases tests` 38 files using an external cache removed after validation; in-memory compile passed. Reverse mutation changing implementation `agents_enabled` to `true` made P1 fail, then was restored. Scope/diff checks, forbidden `Any`/`type: ignore`/broad-catch scan, external-cache readback and worktree Python-cache readback all passed. |
| Return / stop condition | Implementation is `COMPLETED` with the documented typed host-capability outcome. The role-profile product path is fail-closed at `INSTALL_BLOCKED / ROLE_ISOLATION_UNPROVEN`; it must not unblock autonomous Ticket 04, 06B or 06C without a newly authorized capability change and dispatch. No review, integration, push, release or deployment occurred. |
