...
| B1 / B2 evidence | The planner revalidates the integrated 05B1 current journal/request/attempt identity, tables six reachable legal state pairs, returns finite blocks for legal-but-unreachable `(OWNED, PREEXISTING)`, all nine illegal pairs, malformed, cross-request and replay cases. Only `MAY_EXIST`/`OWNED` schedule removals; plugin precedes marketplace, then plugin-list, installed-location and marketplace absence proofs in fixed order. |
| B3 / B4 evidence | The reducer accepts only an exact self-revalidated plan and a complete tuple of normalized outcomes in plan order. Missing, extra, duplicate/reordered, wrong-step, wrong-plan and constructed malformed outcomes return a finite block. Declared removal failures remain as ordered finite reasons while all later proof observations are required. Plugin authority clears only after both plugin-list truths and installed-location truth are freshly absent; marketplace authority clears only on fresh marketplace absence. Non-authority plugin truth is not modeled as owned residue. |
| B5 / metadata | The only results are `COMPENSATION_NOT_REQUIRED`, `COMPENSATED`, `COMPENSATION_FAILED`, and pre-reduction `COMPENSATION_BLOCKED`. Reasons are finite, ordered and unique; serialized results contain no command, stdout, stderr, exception, absolute-location or network data. The reducer accepts no port or callable and executes no effect. |
| Reverse-mutation evidence | (1) Reversing plugin/marketplace removal order made B2 fail. (2) Changing the plugin-list absence conjunction to a disjunction made the B4 truth-table retain no plugin authority for incomplete evidence and fail. (3) Relaxing exact outcome-count equality let a missing sequence reach `zip(strict=True)` and made B3 fail instead of returning the finite sequence block. Each isolated mutation was restored before green verification. |
| Verification | Focused `python -m unittest tests.test_codex_compensation_reducer`: 7/7 passed. Full `python -m unittest discover -s tests`: 237/237 passed. Strict `mypy --strict --explicit-package-bases` over `library` and `tests`: 112 source files, success with validated repository-external caches removed and read back absent. In-memory compile: 112 Python files passed. Source sentinel, exact two-path implementation scope and `git diff --check` passed. |
| Residue / non-interference | Admission verified the prior 06A head, exact reviewed handoff, three-worktree topology and new-branch absence; the sole inherited `.mypy_cache` and all verification-only bytecode/cache directories were removed with exact-target readback. After the implementation commit, tracked and ignored state were clean. No command, process, filesystem, host, Codex, target-project, network, Secret, integration, review, downstream dispatch, push, release or deployment effect occurred. |
...
| Binding | Correction handoff `hnd_local_orchestration_install_05b3b_r02_20260812`; retained allocation `aln_local_orchestration_install_05b3b_20260811`; retained receipt `rcpt_local_orchestration_install_05b3b_20260811`; correlation `corr-local-orchestration-install-05b3b-r02-20260812`; question `q-local-orchestration-install-05b3b-r02-20260812`; side-context `scx-local-orchestration-install-05b3b-20260812-02`; refrozen ticket `0104372e604d93ab47bc456214d8277b6bb12db5`; review `14fda317538f6661573cf687468f5291ced84ff7`. |
...
| First-red evidence | Before correction, the refrozen focused suite failed its new exact proof-order and residual-journal assertions; the wrong-plan metaclass probe reached revision-01 `type(plan) in (...)` and its `KeyboardInterrupt` equality trap escaped rather than returning `PLAN_INVALID`. |
| R1 / CR-140 | The exact plan tuple is now optional plugin removal, optional marketplace removal, `PROVE_PLUGIN_LISTS_ABSENT`, `PROVE_MARKETPLACE_ABSENT`, `PROVE_INSTALLED_LOCATION_ABSENT`, for every reachable authority pair. No-authority and pre-existing pairs remain no-step plans. |
| R2 / CR-141 | Every completed result now carries strict `CodexCompensationResidualJournal` with the exact request and attempt identity plus marketplace/plugin states. Only fresh proof for that effect changes `MAY_EXIST` or `OWNED` to `NOT_ATTEMPTED`; `PREEXISTING` and `NOT_ATTEMPTED` remain unchanged. The old effect-only view is derived from, not serialized instead of, the residual journal, so unresolved `MAY_EXIST` and `OWNED` serialize differently. Cross-request, replay, substituted-state and stale-plan cells block finitely. |
| R3 / CR-142 | Plan class admission uses only `type(plan) is ...` identity checks. Plan reconstruction compares trusted fields without plan-object equality. Wrong-plan metaclasses raising `RuntimeError`, `MemoryError`, `KeyboardInterrupt` or `SystemExit` all now return finite `COMPENSATION_BLOCKED / PLAN_INVALID` without invoking equality. |
| R4 / CR-143 reverse evidence | Six isolated reversals each turned its named test red and were restored: pre-existing marketplace authority (`test_r4_preexisting_authority_never_schedules_removal`); frozen proof order (`test_r1_exact_plan_order_is_frozen_for_every_reachable_authority_pair`); retaining authority after an otherwise complete declared failure (`test_r4_complete_after_declared_failure_reduces_later_proofs`); early plugin clear without installed-location absence (`test_r4_early_plugin_clear_requires_every_fresh_plugin_proof`); never clearing fresh authority (`test_r4_stale_authority_clears_only_after_exact_fresh_absence`); and tuple-membership equality in plan admission (`test_r3_wrong_plan_metaclass_equality_traps_block_without_equality`). |
| R5 / verification | Focused `python -m unittest tests.test_codex_compensation_reducer`: 15/15 passed. Full `python -m unittest discover -s tests`: 245/245 passed. Strict `mypy --strict --explicit-package-bases` over `library` and `tests`: 112 source files, success with repository-external caches removed and read back absent. In-memory compile: 112 Python files passed. Source sentinel, exact scope and `git diff --check` passed. |
| Full-suite staging note | The first full attempt observed one `%TEMP%` marker-bound `johnny-stage-env-*` root in an existing bounded-runner assertion. A read-only follow-up found it already absent; no external staging root was manually removed. The rerun first read zero such roots and then passed all 245 tests. |
| Residue / non-interference | Own-worktree `.mypy_cache`, `.pytest_cache` and `__pycache__` directories were removed only after exact containment checks; tracked and ignored readback is clean. This pure reducer accepted no port/callable and made no live Codex, host, filesystem, target-project, network, Secret, review, integration, dispatch, push, release or deployment effect. |
...
| Real first-red evidence | Before production correction, `test_i2_all_four_plain_object_identity_fields_block_finitely` exercised all four nested identity fields and exited nonzero. The `marketplace_state` and `plugin_state` plain-object cells escaped from `_plan_matches_rebuild()` as `PydanticSerializationError`; the request/attempt cells already returned finite blocks. No later green result is represented as first-red evidence. |
| I1 / I2 correction | Plan admission now verifies the exact concrete type of the identity and each of its four frozen nested fields before rebuild, equality, formatting or serialization. The committed I2 table covers exactly four fields crossed with missing, `None`, empty string, whitespace, list, dict and unrelated plain object: all 28 cells return metadata-only `COMPENSATION_BLOCKED / PLAN_INVALID`. Exact valid identity still admits; four exact-type value substitutions rebuild to mismatch and block. |
| I3 trap matrix | The committed table crosses all four identity fields with `RuntimeError`, `MemoryError`, `KeyboardInterrupt` and `SystemExit` operation traps: all 16 cells return `PLAN_INVALID`, and equality, hash, string, representation and format invocation count remains zero. No trap or process-control exception is caught or invoked. |
| I4 reverse-mutation evidence | (1) Removing only the recursive `marketplace_state` type guard made the 28-cell test fail with one `PydanticSerializationError` and a 27/28 cell count. (2) Moving supplied identity serialization before admission and outside the finite boundary made both state plain-object cells escape and the count fall to 26/28. Each isolated mutation was restored before final green verification. |
| I5 / parent preservation | Focused `python -B -m unittest tests.test_codex_compensation_reducer -v`: 18/18 passed, including every parent B1-B5 and R1-R5 test. Full `python -B -m unittest discover -s tests`: 248/248 passed. Strict full-tree `mypy --strict --explicit-package-bases --no-incremental`: 112 source files passed with an exact repository-external cache removed and read back absent. In-memory compile: 112 Python files. Production source sentinel, exact ancestry/scope and `git diff --check` passed. |
| Residue / non-interference | Before the implementation commit, repository `.mypy_cache`, `.pytest_cache` and `__pycache__` counts were zero; tracked scope contained only the two authorized paths and ignored-only readback was empty. After commit, tracked and ignored readback were clean. The reducer remains pure: no port/callable, command, live Codex, host, target-project, network, Secret, cross-lane read, integration, downstream dispatch, push, release or deployment effect occurred. |
