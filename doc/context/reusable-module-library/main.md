...
| 共同基準 | `d03880e`（Ticket 03 實作）與本次 handoff docs-only commit |
...
- 共用基準實作 commit：`d03880e`（`feat: add deterministic NLP rule parser`）。
...
- Ticket 01、Ticket 02 與 Ticket 03 已完成；下一張 `04-python-nlp-provider-boundaries` 仍須使用者明確確認。
