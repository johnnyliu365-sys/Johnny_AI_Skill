...
| [08-isolated-publication-live-cutover.md](08-isolated-publication-live-cutover.md) | `BLOCKED / DEPENDS_ON_09` | A real isolated Claude install from README reaches only the publication object graph. | `C = 758a718` fails Ticket 06 L2 because its self-pin carrier is intentionally neutralized by the generator but not by the closure verifier. No source/publication ref was pushed. |
| [09-pin-carrier-closure-normalization.md](09-pin-carrier-closure-normalization.md) | `APPROVED / DISPATCH_READY` | The Ticket 06 closure treats the self-pin carrier by the same strict, reversible normalizer as the generator. | Ticket 08 L2 blocker; local-only HIGH_ASSURANCE correction with no release effect. |
