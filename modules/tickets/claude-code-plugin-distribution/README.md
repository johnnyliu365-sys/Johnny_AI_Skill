...
| [08-isolated-publication-live-cutover.md](08-isolated-publication-live-cutover.md) | `BLOCKED / OWNER_EFFECT_AUTHORITY_REQUIRED / VERSION_SPECIFIC_TAG_CLOSURE_COMPLETE` / `CLOSURE_04` | A real isolated Claude install from README reaches only the publication object graph. | Ticket 12 integrated the local multi-release closure at `9a244db`; the former CLOSURE_03 effect record remains suspended. A new exact Ticket 08 owner effect authority is required before any candidate, ref, tag, cache or CLI action. |
