...
| [08-isolated-publication-live-cutover.md](08-isolated-publication-live-cutover.md) | `BLOCKED / REQUIREMENT_CHANGED / MULTI_RELEASE_TAG_DECLARATION_CONFLICT` / `CLOSURE_03` | A real isolated Claude install from README reaches only the publication object graph. | F3 made the current payload differ from immutable `plugin-v0.4.10`; L2 currently compares every tag to one current declaration, so no multi-release ref set can pass. No external effect occurred; a version-specific historical-tag verification decision is required. |
