...
| [08-isolated-publication-live-cutover.md](08-isolated-publication-live-cutover.md) | `READY / OWNER_EFFECT_AUTHORITY_RECORDED / CLOSURE_04` | A real isolated Claude install from README reaches only the publication object graph. | Fresh owner authority binds the `0.4.11` branch, temporary raw-descriptor ref, correlation and the read-only `b52215e` publication snapshot. Terra must still bind a reviewer-written candidate/root and re-read the remote before every effect. |
