...
| [08-isolated-publication-live-cutover.md](08-isolated-publication-live-cutover.md) | `BLOCKED / TICKET_DEFECT / UPSTREAM_CLOSURE_CONTRACT` / `CLOSURE_02` | A real isolated Claude install from README reaches only the publication object graph. | The real cache contains normal `origin/HEAD -> origin/main`; the Ticket 06 installed-cache parser rejects that symbolic ref before L4 closure. Publication root/tag and the temporary source candidate are preserved; source `main` is not integrated. |
...
| [10-installed-cache-symbolic-remote-head.md](10-installed-cache-symbolic-remote-head.md) | `BLOCKED / CONVERGENCE_REVIEW_REQUIRED` / `CLOSURE_01` | A normal clone's symbolic remote default head is admitted only when it is strictly bound to the same remote `main` root and payload closure. | The candidate still accepts raw CRLF because universal-newline reading normalizes it. One correction has already been reviewed; closure revision and owner approval are required before a further cycle. |
