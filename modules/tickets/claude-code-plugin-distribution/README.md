...
| [14-synchronous-dispatch-guidance-release.md](14-synchronous-dispatch-guidance-release.md) | `APPROVED_NOT_DISPATCHED / OWNER_EFFECT_AUTHORITY_RECORDED` | The shipped guidance distinguishes same-lifetime synchronous dispatch from the optional receipt-bound cross-lifetime bridge. | Corrects the ADR-014 follow-up that was never opened. The owner authorized one `0.4.12` publication candidate against the recorded `0.4.11` remote snapshot; no effect may occur until the reviewer binds and verifies the exact generated root. |
