...
| [11-level-one-payload-topology.md](11-level-one-payload-topology.md) | `OPEN / OWNER_APPROVED / DISPATCH_AUTHORIZED` — `F3` | The Level 1 declaration is exactly the reachable reusable-source surface, excluding host-local publication/cache/installer tooling. | `POC / STANDARD`; local source/declaration closure only. Its integration is the prerequisite to owner successor-version selection and any Ticket 08 re-admission. |
