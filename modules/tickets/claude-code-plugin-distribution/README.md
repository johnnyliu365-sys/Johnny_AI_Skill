...
| [10-installed-cache-symbolic-remote-head.md](10-installed-cache-symbolic-remote-head.md) | `DRAFT / PENDING_OWNER_APPROVAL` | A normal clone's symbolic remote default head is admitted only when it is strictly bound to the same remote `main` root and payload closure. | Corrects the Ticket 08 L4 checker defect with local fixtures only; no remote, cache or Claude effect. On completion, Ticket 08 requires fresh re-admission and all L1–L6 evidence. |
