...
| [10-installed-cache-symbolic-remote-head.md](10-installed-cache-symbolic-remote-head.md) | `APPROVED / DISPATCH_READY` / `CLOSURE_02` | A normal clone's symbolic remote default head is admitted only when it is strictly bound to the same remote `main` root and payload closure. | Owner-approved convergence revision: raw CRLF and trailing-whitespace loose targets must fail from raw bytes. One additive correction/review cycle is authorized; no external effect. |
