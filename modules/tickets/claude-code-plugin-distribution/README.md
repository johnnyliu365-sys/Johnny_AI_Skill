...
| [17-codex-v0414-fresh-release-readmission.md](17-codex-v0414-fresh-release-readmission.md) | `APPROVED_NOT_DISPATCHED / CLOSURE_01` | The already-created immutable `0.4.14` publication object is freshly verified and its reviewed development candidate is integrated only after Ticket 16 closes. | `POC / HIGH_ASSURANCE`; Codex uses a disposable Git marketplace clone because its effective CLI has no raw-descriptor source form, while plugin-cache closure is verified against the independent publication repository. The final candidate uses the ticket-named r03 temporary ref; no existing ref/tag is moved or deleted. |
