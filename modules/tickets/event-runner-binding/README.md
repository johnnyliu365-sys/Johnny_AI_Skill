...
| E14 | Claude branch wake command | The Router drives Claude conversation branches, one per reviewer; see [`e14-claude-branch-wake-command.md`](e14-claude-branch-wake-command.md) | `PARTIALLY CLOSED` — dispatcher ships with per-attempt routing and a probe that requires a completed turn; 1014 passed, zero residue. R8 (live drive) open: the host CLI reported `loggedIn: false` throughout, so no cell has driven a real branch |
