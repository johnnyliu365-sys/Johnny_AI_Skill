...
- **雷**：2026-08-20，完整套件出現 `2 failed`，其一是
  `test_codex_lifecycle_oracle.py::test_p5_foreign_only_presence_evidence_is_absent_and_byte_preserving`
  （用 lease 配置器的 cell）。**另一個是什麼不知道**——當時把輸出接了 `tail -2`，
  正是 D4 明文禁止的事，第二筆就此遺失。
- **重現嘗試（都失敗）**：oracle 單檔 ×5 全綠；oracle ＋ 當時新增的並行測試
  ×5 全綠；完整套件在「疑似成因未修正」的狀態下 ×2 全綠；修正後 ×2 全綠。
  合計 1 紅、13 綠。
- **狀態**：**未結案。成因不明，且沒有任何修法被證明有效。**
  `test_parallel_worker_dispatch.py` 的子行程已改為在暫存目錄下執行（不再繼承
  checkout 當 cwd），但那是衛生問題，**不得當成此雷的修法**——它從未被證明相關。
- **下次發生時要做的事**：不要只看結尾。完整保留
  `FAILED`／`SUBFAILED` 全部行、`-p no:randomly` 的實際順序、以及
  `tests/.johnny-runtime` 當下的內容。在拿到這些之前，任何「已修好」的說法都是猜的。
- **教訓**：`1 紅 N 綠` 對間歇性失敗不構成證據——間歇性失敗的定義就是大多數時候是綠的。
  緩解不是診斷；把緩解講成修法，就是把一個活著的缺陷從帳上抹掉。
