...
| [09b2-recoverable-managed-artifact-writer](09b2-recoverable-managed-artifact-writer.md) | One local runtime writer persists private recovery evidence before effects, revalidates under an advisory lock, and fails closed until explicit recovery proves settlement. | `BLOCKED / NO_SUPPORTED_ATOMIC_CONDITIONAL_REPLACE_PROOF / ARCHITECTURE_DECISION_REQUIRED / NOT_INTEGRATED` | Windows/NTFS, Linux/WSL DrvFS and current CPython/NTFS all qualified `NO` at `5763caf`; do not correct `f99d836` |
...
deviation. `CAP-RWW6-01` is complete: every executed tuple qualified `NO`. R09B2 is now blocked
for a second architecture/SPEC decision; neither the investigation nor `f99d836` may be treated as
repair authority. R09C–R09E and publication remain unopened.
