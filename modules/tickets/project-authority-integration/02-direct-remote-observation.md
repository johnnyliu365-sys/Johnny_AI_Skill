...
preflight, reviewer mutation, and fake-only evidence. A same-lifetime reviewer allocation binds
task/worktree/branch/correlation without a receipt, live descriptor, or host gateway; only a
cross-lifetime handoff uses those additional controls. It must not infer authority from main,
origin, fetch, CI, PR, provider UI, or a process exit.
