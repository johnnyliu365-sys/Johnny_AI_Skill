...
- Corrective implementation commits: `eb4bb8f` (`fix: halt blocked implementation returns`) and `7faa590` (`fix: preserve legacy completion rules`). Earlier `d7e40cf` / `8bd984c` / `423c41b` were externally advanced during the first provenance audit; these corrections are independently authored in the assigned implementation worktree.
