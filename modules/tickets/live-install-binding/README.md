...
| L11 | [One-click console-stream observability](l11-one-click-console-stream-observability.md) | Windows refusal tests observe the complete decoded console stream, while the wrapper hashes through .NET rather than an auto-loaded PowerShell module. | `CONVERGENCE_REVIEW_REQUIRED / NOT_DISPATCHED / REVISION_02` — the prior test-only assumption was invalid: `Get-FileHash` was unavailable before L9 could emit its named refusal. |
