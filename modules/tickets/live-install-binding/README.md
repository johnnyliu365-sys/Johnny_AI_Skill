...
| L11 | [One-click console-stream observability](l11-one-click-console-stream-observability.md) | Windows refusal tests observe the complete decoded console stream, while the wrapper hashes through .NET rather than an auto-loaded PowerShell module. | `CONVERGENCE_REVIEW_REQUIRED / NOT_DISPATCHED / REVISION_03` — corrected wrapper output may be stdout or stderr; the test observes both without making placement a contract. |
