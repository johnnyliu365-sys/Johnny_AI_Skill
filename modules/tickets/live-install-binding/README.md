...
| L11 | [One-click console-stream observability](l11-one-click-console-stream-observability.md) | Windows refusal tests observe the complete decoded console stream, the wrapper hashes through .NET, and its finite unreadable-digest refusal is guidance-classified. | `CONVERGENCE_REVIEW_REQUIRED / NOT_DISPATCHED / REVISION_04` — the L11 guidance partition must close independently; Ticket 08's unintegrated pin/tree failures remain its own release blocker. |
