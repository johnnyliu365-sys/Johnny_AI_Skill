...
| `A7` | One real project-owned disposable-runtime scenario initializes/adds/removes/lists/proves absence through the exact oracle and adapter, preserves seeded foreign record/payload bytes, removes only owned state and leaves teardown to the environment owner. No live Codex or target project is touched. |
...
`doc/WorkProgressReport.md`-only handoff reserved as `PRG-20260814-348`.
...
| Writable return | Exactly the two frozen new paths, one implementation commit, then only PRG-348 in one WPR-only handoff commit. |
