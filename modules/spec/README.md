...
| Context-load telemetry | [`context-load-telemetry.md`](context-load-telemetry.md) Revision 10 | `APPROVED / REVIEWER_DECOMPOSITION_AUTHORIZED`；opaque Johnny-owned storage、per-stream ledger/CAS、lock-bound transaction adapter protocol and its no-effect private composition binding are defined. The composition ticket may be opened; provider probes and release remain separately gated. |
