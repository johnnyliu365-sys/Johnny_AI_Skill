...
- 狀態：`REVISION_02 / ROUTER_PHASE_APPROVED / OTHER_PHASES_OWNER_REVIEW_REQUIRED`。
- 來源：`CHG-20260813-016`、`CHG-20260813-017`、`CHG-20260814-019`、`ADR-20260813-008`、`ADR-20260813-009`、`ADR-20260814-011`、`PRD.md §17`、`doc/context/adaptive-project-orchestration/main.md`。
...
- 模型生命週期：最高能力 architecture owner 與人類 owner 關閉並核准 SPEC；readiness gate 通過後交給 Terra supervisor/reviewer 拆票與審閱，預設 Luna implementer 只執行 admitted ticket。語意／契約／架構／AC／assurance 未關閉時由 Router 喚醒 architecture owner；model name 不授予 authority。
- 拆票 closure：一個可觀察結果、一個 owner、一個主要 change/effect boundary、有限 TDD、確定性驗證、零 unresolved design decision；依 behavior/state/effect/ownership/verification 拆分，不依檔案或行數。
- UI design source：Figma、截圖、design brief 與既有 design system 都可成為核准輸入；Figma 不強制安裝。只有 runtime 不可信資料進 renderer/JavaScript sink 才觸發 XSS 路由。
...
- 邊界：目前只授權 Router policy phase；06G0P 已完成回傳但暫不 review/integrate，06G0-06G4 與其他返工／新功能票暫停。不移動既有 worktree，不授權 target-project mutation、push、package、release 或 deployment。
