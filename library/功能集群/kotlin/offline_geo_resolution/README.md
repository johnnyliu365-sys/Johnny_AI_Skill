...
5. 建表時略過 validator 拒絕的座標；相同精確 key 保留第一筆有效座標，以對齊參照 resolver 的 `putIfAbsent` 語義。
...
```kotlin
val resolver = OfflineGeoResolver.fromEntries(
    entries = localEntries,
    addressKeyPolicy = projectAddressKeyPolicy,
    coordinateValidator = projectCoordinateValidator,
)

when (val result = resolver.resolve(RawAddressKey("safe-local-key"))) {
