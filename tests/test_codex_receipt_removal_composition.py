...
class CodexReceiptRemovalCompositionTests(unittest.TestCase):
    ...
    def test_CR176_actual_observer_mismatch_and_unproved_preproofs_block(self) -> None:
        cases: tuple[tuple[str, object, object, object], ...] = (
            (
                "plugin-mismatch",
                _mismatched_plugins(),
            ...
            (
                ...
                _empty_plugins(),
                _empty_marketplaces(),
                _path(True, manifest=_foreign_manifest()),
            ),
            (
                "plugin-unproved",
                _failure(CodexCompensationPortOperation.LIST_PLUGINS),
            ...
            (
                "marketplace-unproved",
                _empty_plugins(),
                _failure(CodexCompensationPortOperation.LIST_MARKETPLACES),
                _path(True),
            ),
            (
                "path-unproved",
                _empty_plugins(),
                _empty_marketplaces(),
                _failure(CodexCompensationPortOperation.PROVE_INSTALLED_PATH_ABSENT),
            ),
        )
        expected_preproof_calls = [
            CodexCompensationPortOperation.LIST_PLUGINS,
            CodexCompensationPortOperation.LIST_MARKETPLACES,
            CodexCompensationPortOperation.PROVE_INSTALLED_PATH_ABSENT,
        ]
        for name, plugins, marketplaces, path in cases:
            with self.subTest(name=name):
                port = _RecordingPort(
                    ...
                    marketplace_lists=[marketplaces],
                    paths=[path],
                )
                result = compose_codex_receipt_removal(_invocation(), port)
                self.assertIsInstance(result, CodexReceiptRemovalCompositionBlocked)
                ...
                self.assertEqual(expected_preproof_calls, port.calls)
                self.assertEqual(
                    [],
                    [
                        call
    ...
    def test_B7_receipt_identity_and_invalid_port_are_finite(self) -> None:
        ...
        self.assertEqual(0, identity_trap.inspections)
        self.assertEqual([], identity_trap.hooks)
    ...
    def test_CR176_invalid_capability_has_zero_operations_and_no_hooks(self) -> None:
        trap = _CandidateTrap()
        result = compose_codex_receipt_removal(_invocation(), trap)
        self.assertIsInstance(result, CodexReceiptRemovalCompositionBlocked)
        self.assertEqual(
            CodexReceiptRemovalCompositionBlockReason.INVALID_PORT,
            cast(CodexReceiptRemovalCompositionBlocked, result).reason,
        ...
        self.assertEqual(0, trap.inspections)
        self.assertEqual([], trap.hooks)
        self.assertEqual([], trap.operations)
    ...
    def test_B7_invalid_receipt_is_finite_before_port_admission(self) -> None:
        ...
        trap = _CandidateTrap()
        result = compose_codex_receipt_removal(invocation, trap)
        self.assertIsInstance(result, CodexReceiptRemovalCompositionBlocked)
        self.assertEqual(
...
def _mismatched_plugins() -> CodexPluginList:
    mismatched = CodexPluginEntry(
        pluginId=PLUGIN_ID.value,
    ...
    return CodexPluginList(installed=(mismatched,), available=())
...
def _mismatched_marketplaces() -> CodexMarketplaceList:
    mismatched = CodexMarketplaceEntry(
        name=MARKETPLACE.value,
        root=r"C:\disposable\probe-market",
        marketplaceSource=CodexMarketplaceSource(
            type="directory",
            value="marketplaces/different-market",
        ),
    )
    return CodexMarketplaceList(marketplaces=(mismatched,))
...
def _foreign_manifest() -> CodexCompensationPortManifest:
    return CodexCompensationPortManifest(
        ...
        plugin=CodexPluginName(value="foreign-plugin"),
        version=VERSION,
        installed_locator=INSTALLED,
        auth_policy=AUTH_POLICY,
        digest=DIGEST,
...
class _CandidateTrap:
    ...
    def __getattribute__(self, name: str) -> object:
        if name not in {
            ...
            object.__getattribute__(self, "hooks").append(name)
    ...
    def __eq__(self, other: object) -> bool:
        object.__getattribute__(self, "hooks").append("equality")
        raise AssertionError("candidate equality invoked")

    def __repr__(self) -> str:
        object.__getattribute__(self, "hooks").append("representation")
        raise AssertionError("candidate representation invoked")

    def __str__(self) -> str:
        object.__getattribute__(self, "hooks").append("string")
        raise AssertionError("candidate string invoked")

    def model_dump_json(self) -> str:
        object.__getattribute__(self, "hooks").append("serialization")
        raise AssertionError("candidate serialization invoked")
